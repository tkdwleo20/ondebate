...
returns void language plpgsql security definer set search_path = public as $$
declare v_user_id uuid; v_is_anonymous boolean;
begin
  ...
  select r.reported_user_id, coalesce((select m.is_anonymous from public.debate_messages m where m.debate_id = r.debate_id and m.author_id = r.reported_user_id order by m.created_at limit 1), false)
  into v_user_id, v_is_anonymous from public.reports r where r.id = p_report_id and r.status = 'open';
